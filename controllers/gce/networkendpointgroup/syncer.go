...
func (s *syncer) sync() error {
	...
	glog.V(2).Infof("=======target map: %+v\n current map: %+v", targetMap, currentMap)
	...
	glog.V(2).Infof("=======addEndpoints map: %+v\n removeEndpoints map: %+v", addEndpoints, removeEndpoints)
...
func (s *syncer) syncNetworkEndpoints(addEndpoints, removeEndpoints map[string]sets.String) error {
	glog.V(2).Infof("============================syncNetworkEndpoints start for %s/%s", s.namespace, s.name)
	...
	glog.V(2).Infof("============================syncNetworkEndpoints waiting for %s/%s", s.namespace, s.name)
	wg.Wait()
	glog.V(2).Infof("============================syncNetworkEndpoints finished for %s/%s", s.namespace, s.name)
