...
// syncService takes a service and determines whether it needs NEGs or not.
...
// Remove dangling NEG belong to the cluster and clean up syncerMap
...
func isNEGEnabled(service *apiv1.Service) bool {
	v, ok := service.Annotations[networkEndpointGroupAlphaAnnotation]
	if ok && v == "true" {
		return true
	}
	return false
